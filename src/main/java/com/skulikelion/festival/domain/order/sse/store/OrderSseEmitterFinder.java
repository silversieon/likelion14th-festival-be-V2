...
public interface OrderSseEmitterFinder {
   ...
   */
  Map<OrderSseSubscribeType, List<SseEmitter>> findByBoothId(Long boothId);

  /**
   * [ 부스 내 특정 구독 타입 Emitter 조회 ] 부스 식별자와 구독 타입으로 등록된 모든 Emitter 조회
   *
   * @param boothId 부스 식별자
   * @param orderSseSubscribeType 구독 타입 (주문 상태)
   * @return 특정 구독 타입의 모든 Emitter 객체
   */
  List<SseEmitter> findByBoothIdAndSubscribeType(
